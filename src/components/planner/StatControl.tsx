...
export default function StatControl({ label, value, bonus = 0, onIncrease, onDecrease, cost }: Props) {
  return (
    <div style={{ background: "white", borderRadius: "12px", padding: "12px 16px", border: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <div>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "4px" }}>
          ...
          {bonus > 0 && <span style={{ fontSize: "1rem", fontWeight: 700, color: "#22c55e" }}>+ {bonus}</span>}
          {bonus < 0 && <span style={{ fontSize: "1rem", fontWeight: 700, color: "#ef4444" }}>- {Math.abs(bonus)}</span>}
        </div>
      </div>
